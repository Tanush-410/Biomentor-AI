...
def _generate_with_ai(question: str, contexts, conversation_history):
    ...
    system_prompt = (
                    ...
                    "3) Proactively include a real picture whenever the topic is something a visual genuinely helps "
                    "with -- anatomy, organisms, real-world objects, historical figures or places, lab equipment, "
                    "physical structures, and similar -- even if the student did not explicitly ask for one. You do "
                    "not need to wait for words like 'picture', 'photo', or 'image' before including one; use your "
                    "judgment the way a good textbook does, adding an illustration wherever it would help the "
                    "student understand faster, and skipping it only for purely abstract, numeric, or definition-only "
                    "questions where a picture would not add anything. When you include one, request it with a "
                    "fenced ```image code block. That query is "
